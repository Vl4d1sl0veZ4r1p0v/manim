...
class MoreShapes(Scene):
    ...
    def construct(self):
        ...
        house = Polygon(np.array([0, 2, 0]), np.array([1, 3, 0]), np.array([2, 2, 0]), np.array([2, 0, 0]), np.array([0, 0, 0]), color=LIGHT_BROWN, fill_color=PINK, fill_opacity=1)
        circle = Circle(color=PINK)
        circle.surround(house)

        self.play(GrowFromCenter(house))
        self.play(FadeIn(circle))
        self.play(ApplyMethod(house.move_to, LEFT_SIDE))
        self.play(ApplyMethod(house.shift, DOWN))
        house.match_color(circle)
        self.play(FadeOut(house))
...
class AddingText(Scene):
    ...
    def construct(self):
        ...
        third_line = TextMobject(r"\[\Delta = b^2 - 4ac\]")
...
class AddingMoreText(Scene):
    ...
    def construct(self):
        ...
        # self.play(ApplyMethod(circle.next_to, square, DOWN))
        # self.play(ApplyMethod(circle.move_to, ORIGIN))
        # self.play(ApplyMethod(circle.to_edge, RIGHT_SIDE))
        # self.play(ApplyMethod(circle.move_to, LEFT_SIDE))
        # self.play(ApplyMethod(circle.to_edge, TOP))
        self.play(ApplyMethod(circle.move_to, square.get_corner(UP+LEFT)+UP))
        self.wait(3)
...
class RotateAndHighlight(Scene):
    ...
    def construct(self):
        ...
        label.bg = BackgroundRectangle(label, fill_opacity=.5, color=PINK)
        label_group = VGroup(label.bg, label)  # Order matters
        label_group.rotate(TAU / 8)
        label2 = TextMobject("Boxed text", color=WHITE)
        ...
        label3.scale(2)
        label3.set_color_by_gradient(YELLOW, GREEN)
        label3.to_edge(DOWN)
        ...
        self.play(Rotate(label2.bg))
        self.play(ApplyMethod(label2.bg.shift, UP))
        self.play(ApplyMethod(label3.move_to, RIGHT_SIDE))
        self.play(ApplyMethod(label3.move_to, LEFT_SIDE))
        self.wait(3)
...
class ColoringEquations(Scene):
    ...
    def construct(self):
        # line1 = TexMobject(r"\text{The vector } \vec{F}_{net} is the net", r"\text{force }",
        ...
        # way equasion form physics

        line = TexMobject(r"S = ", "V_0", "t", r" + \frac{g t^{3}}{2}")
        line.set_color_by_tex("V_0", PINK)
...
class UsingBracesConcise(Scene):
    ...
    def construct(self):
        ...
        eq1 = ['-', '1', 'x', '+', '2', 'y', '-', '3', 'z', '=', '-6']
        eq2 = ['0', 'x', '-', '1', 'y', '+', '5', 'z', '=', '0']
        eq1_mob = TexMobject(*eq1)
        eq2_mob = TexMobject(*eq2)
        eq1_mob.move_to(UP+LEFT)
        eq2_mob.move_to(UP+LEFT)
        eq1_mob.set_color_by_tex_to_color_map({
            'x': RED,
            'y': YELLOW,
            'z': GREEN,
        ...
        for i, item in enumerate(eq2_mob):
            item.align_to(eq1_mob[i], np.array([0, 0, 0]))
        eq1 = VGroup(*eq1_mob)
        eq2 = VGroup(*eq2_mob)
        eq2.shift(DOWN)
        eq_groups = VGroup(eq1, eq2)
        braces = Brace(eq_groups, LEFT)
        eq_text = braces.get_text("A pair of equations")
        circle = Circle(color=PINK)
        circle.surround(eq_groups)
        self.play(FadeIn(circle))
        self.play(Write(eq1), Write(eq2))
        self.play(GrowFromCenter(braces), Write(eq_text))
